...
#        filters=Filters.command([f"{GLEECH@Leeech2vidbot}"]) & Filters.chat(chats=AUTH_CHANNEL)
#    )
...
#    app.add_handler(incoming_telegram_download_handler)
    ...
    incoming_purge_message_handler = MessageHandler(
        ...
        filters=Filters.command(["purge@Leeech2vidbot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
...
#    app.add_handler(incoming_size_checker_handler)
    ...
    exec_message_handler = MessageHandler(
        exec_message_f,
        filters=Filters.command(["exec@Leeech2vidbot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    ...
    eval_message_handler = MessageHandler(
        eval_message_f,
        filters=Filters.command(["eval@Leeech2vidbot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    ...
    rename_message_handler = MessageHandler(
        rename_message_f,
        filters=Filters.command(["rename@Leeech2vidbot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(rename_message_handler)
    #
    upload_document_handler = MessageHandler(
        upload_document_f,
        filters=Filters.command(["upload@Leeech2vidbot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
    app.add_handler(upload_document_handler)

    help_text_handler = MessageHandler(
        help_message_f,
        filters=Filters.command(["help@Leeech2vidbot"]) & Filters.chat(chats=AUTH_CHANNEL)
    )
